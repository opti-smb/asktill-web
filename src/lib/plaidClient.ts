...
export function findSameLinkedBank(
  ...
  return accounts.find((account) => {
    ...
    return Boolean(
      name && (account.institution_name || '').trim().toLowerCase() === name,
    );
  });
}
...
/** Plaid Link onSuccess metadata (subset used for duplicate detection). */
...
function accountTypeKey(type: string | null | undefined, subtype: string | null | undefined): string {
  return (subtype || type || '').trim().toLowerCase();
}

function institutionNameKey(name: string | null | undefined): string {
  return (name || '').trim().toLowerCase();
}

function duplicateAccountLabel(account: LinkedBankAccount): string {
  const name = account.name?.trim() || account.institution_name?.trim() || 'This bank account';
  const mask = account.mask ? ` ···${account.mask}` : '';
  return `${name}${mask}`;
}

/** Block linking an account that is already connected (client-side, before /exchange). */
export function findDuplicatePlaidLink(
  ...
  const sameBank = findSameLinkedBank(
    existing,
    metadata?.institution?.institution_id,
    metadata?.institution?.name,
  );
  if (sameBank) {
    return alreadyLinkedBankMessage(
      sameBank.institution_name || metadata?.institution?.name,
    );
  }

  if (!metadata?.accounts?.length) return null;

  const incomingInstName = institutionNameKey(metadata.institution?.name);
  ...
  for (const acct of metadata.accounts) {
    ...
    for (const row of existing) {
      ...
      const rowInstName = institutionNameKey(row.institution_name);

      const institutionMatch =
        (incomingInstName && rowInstName && incomingInstName === rowInstName)
        || false;
...
export async function plaidJson<T>(
  ...
  const data = (await res.json().catch(() => ({}))) as T & { error?: string; warnings?: unknown[] };
  if (!res.ok) {
    const detail =
      (typeof data.error === 'string' && data.error.trim()) ||
      (Array.isArray(data.warnings) && data.warnings[0] && typeof data.warnings[0] === 'object'
        ? String((data.warnings[0] as { warning?: string }).warning || '')
        : '') ||
      `Bank link request failed (${res.status})`;
